...
simu_mnar_1grp <- function(n, delta_mnar, trunc_time, lambda, beta, lambda_cen, beta_cen, lambda_control = NULL, beta_control = NULL, seed = NULL){
  ...
  v  <- runif(n=n)
  c.time  <- (- log(1-v) / (lambda_cen * exp(x * beta_cen)))
  c.time <- pmin(trunc_time, c.time)
  ...
  if(is.null(beta_control)){
    .time <- (- log(1-v) / (delta * lambda * exp(x * beta)))
  }else{
    .time <- (- log(1-v) / (delta * lambda_control * exp(x * beta_control)))
  }
  t.time <- ifelse(pattern %in% c(1,2), t.time, c.time + .time)
