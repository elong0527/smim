...
main<-function(seed){
  ...
  res.out<-fctn_est(U,R,n,xx1,eval.times,par.delta)
  betahat1 <- res.out$betahat1
  ve.beta1 <- res.out$ve.beta1
  est1     <- res.out$est1
  se1      <- res.out$se1
  se1.mi   <- res.out$se1.mi
  est.q1   <- res.out$est.q1
  est.q2   <- res.out$est.q2
  est2     <- res.out$est2
  se2      <- res.out$se2
  se2.mi   <- res.out$se2.mi
  tau.q1   <- res.out$tau.q1
  tau.q2   <- res.out$tau.q2
  
  
  ##########################################################

  true.trt0<-fctn_true(n=10000,lambda*par0.trt0,beta,lambdaC,betaC,par.delta=1,eval.times,LL,n.eval) ## true survival curve
  tauLL.trt0<-sum(true.trt0*(LL/n.eval)) ## true restricted mean survival time
  ...
  ## generate T 
  v  <- runif(n=n)
  T  <- (- log(1-v) / (lambda * par0.trt0 * exp(X %*% beta)))
  ## generate C
  v  <- runif(n=n)
  C  <- (- log(1-v) / (lambdaC * exp(X %*% betaC)))
  DeltaT <- (T<tau ) & (T<C)
  ...
  res.out<-fctn_est(U,R,n,xx1,eval.times,1)
  betahat1.trt0 <- res.out$betahat1
  ve.beta1.trt0 <- res.out$ve.beta1
  est1.trt0     <- res.out$est1
  se1.trt0      <- res.out$se1
  se1.mi.trt0   <- res.out$se1.mi
  est.q1.trt0   <- res.out$est.q1
  est.q2.trt0   <- res.out$est.q2
  est2.trt0     <- res.out$est2
  se2.trt0      <- res.out$se2
  se2.mi.trt0   <- res.out$se2.mi
  tau.q1.trt0   <- res.out$tau.q1
  tau.q2.trt0   <- res.out$tau.q2
  
  return(list(true=true,eval.times=eval.times, tauLL=tauLL,
              betahat1=betahat1,ve.beta1=ve.beta1,
              est1=est1,se1=se1,se1.mi=se1.mi,
              est.q1=est.q1,est.q2=est.q2,
              est2=est2,se2=se2,se2.mi=se2.mi,
              tau.q1=tau.q1,tau.q2=tau.q2,
              true.trt0=true.trt0,eval.times=eval.times, tauLL.trt0=tauLL.trt0,
              betahat1.trt0=betahat1.trt0,ve.beta1.trt0=ve.beta1.trt0,
              est1.trt0=est1.trt0,se1.trt0=se1.trt0,se1.mi.trt0=se1.mi.trt0,
              est.q1.trt0=est.q1.trt0,est.q2.trt0=est.q2.trt0,
              est2.trt0=est2.trt0,se2.trt0=se2.trt0,se2.mi.trt0=se2.mi.trt0,
              tau.q1.trt0=tau.q1.trt0,tau.q2.trt0=tau.q2.trt0  ))
}
...
fctn_est<-function(U,R,n,xx1,eval.times,par.delta){
  ...
  return(list(betahat1=betahat1,ve.beta1=ve.beta1,
              est1=est,se1=sqrt(ve),se1.mi=sqrt(ve.adjmi),
              est.q1=est.q1,est.q2=est.q2,
              est2=tau.adj,se2=sqrt(ve.tau),se2.mi=sqrt(ve.taumi),
              tau.q1=tau.q1,tau.q2=tau.q2))
}



fctn_true<-function(n=10000,lambda,beta,lambdaC,betaC,par.delta,eval.times,LL,n.eval){
  tau<-5#runif(n,1.5,5)
  ## generate time-indept covariate
  dimX<-1
  X<-rnorm(n,0,1)
  ...
  #summary(U)
  v  <- runif(n=n)
  T2star <- (- log(1-v) / (par.delta*lambda * exp(X %*% beta)))
  Tstar<-DeltaT*T+(R==1)*T+(R==2)*(C+T2star)
  summary(Tstar)
  true.surv<-apply(matrix(Tstar,nrow=n,ncol=length(eval.times),byrow = FALSE)>=
          matrix(eval.times,nrow=n,ncol=length(eval.times),byrow = TRUE),2,mean)
  true.surv
}


fctn_mple<-function(par,matxx1,matYu,n,ltime,dNu){
  beta<-par
  matSx0 <- exp(matxx1*beta[1])*matYu
  matSx1 <- matxx1*exp(matxx1*beta[1])*matYu
  matSx2 <- matxx1^2*exp(matxx1*beta[1])*matYu
  sx0    <- apply(matSx0,2,mean)
  sx1    <- apply(matSx1,2,mean)
  sx2    <- apply(matSx2,2,mean)
  matsx0 <- matrix(sx0,nrow=n,ncol=ltime,byrow=TRUE)
...
#jj<-multiroot(f = fctn_mple,start = betahat1,matxx1=matxx1,matYu=matYu,n=n,ltime=ltime,dNu=dNu)
